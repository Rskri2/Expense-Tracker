...
exports.createIncCtrl = catchAsync((async(req, res,next) =>{
    const {title,amount,description,category,date} = req.body.income ? req.body.income : req.body;
...
exports.getIncome = catchAsync((async(req, res, next) => {
    ...
    if(!income)return next(new AppError('There is no income associated with this user',404));
...
exports.getAllIncome = catchAsync((async(req, res, next) => {
        ...
        const incomes = await Income.paginate({},{limit : 10, page:Number(page),populate:'user'});
        if(!incomes)return next(new AppError('There is no data in this field',404));
        res.status(200).json({
            size:incomes.length,
            status:'success',
            data:incomes
